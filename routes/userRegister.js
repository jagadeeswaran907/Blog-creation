...
router.post("/user-register", async (req, res) => {
    ...
    await bcrypt.hash(myPlaintextPassword, 0, function (err, hash) {
      ...
      connection.query("call Register_User(?,?)", [req.body.email, hash], (error, results, fields) => {
        if (!error) {
  ...
  router.post("/user-login", async (req, res) => {
    ...
    connection.query("call Login_User(?)", [req.body.email], async (error, results, fields) => {
      if (!error) {
